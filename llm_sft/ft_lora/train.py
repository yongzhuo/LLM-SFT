...
# @time    : 2023/3/5 21:04
# @author  : Mo
# @function: train, ymcui/Chinese-LLaMA-Alpaca
...
from llm_sft.ft_lora.config import CUDA_VISIBLE_DEVICES, USE_TORCH, CPU_NUMS  # from config
...
from llm_sft.ft_lora.config import PATH_MODEL_PRETRAIN, DATA_PATH, MODEL_SAVE_DIR, REPO_ID
from llm_sft.ft_lora.config import MICRO_BATCH_SIZE, BATCH_SIZE, GRADIENT_ACCUMULATION_STEPS
from llm_sft.ft_lora.config import LEARNING_RATE, EPOCHS, SAVE_STEPS, VAL_SET_SIZE, TARGET_MODULES
from llm_sft.ft_lora.config import IS_PARALLELIZABLE, MODEL_PARALLEL, USE_CACHE
from llm_sft.ft_lora.config import MAX_LENGTH_Q, MAX_LENGTH_A, MAX_LENGTH_QA
from llm_sft.ft_lora.config import LORA_DROPOUT, LORA_ALPHA, LORA_R
from llm_sft.ft_lora.config import REPO_ID_LORA, PATH_MODEL_LORA
...
def prepare_model_for_half_training(model, output_embedding_layer_name="lm_head",
    ...
    for name, param in model.named_parameters():
        ...
        elif name in output_embedding_layer_name:  # lm_head也需要是tf.float32(最后一层)
            param.data = param.data.to(torch.float32)
        else:
    ...
    return model
def lora_merge_and_unload(model, first_weight, first_weight_old):
    """   LoRA merge_and_unload   """
    # model = model.cuda()
    # first_weight = first_weight.cuda()
    # first_weight_old = first_weight_old.cuda()
    if hasattr(LoraModel, 'merge_and_unload'):
        ...
        lora_model_sd = torch.load(os.path.join(PATH_MODEL_LORA, 'adapter_model.bin'), map_location='cpu')
        lora_config = LoraConfig.from_pretrained(PATH_MODEL_LORA)
...
tokenizer = LlamaTokenizer.from_pretrained(PATH_MODEL_LORA,  # PATH_MODEL_PRETRAIN,
                                           add_eos_token=True,
...
model = LlamaForCausalLM.from_pretrained(PATH_MODEL_PRETRAIN,
                         trust_remote_code=False,
                         # REPO_ID,
                         # cache_dir=PATH_MODEL_PRETRAIN,
                         # torch_dtype=torch.float16,
                         # load_in_8bit=True,
                         # device_map="auto",
                         # device_map={"": int(CUDA_VISIBLE_DEVICES)},
                         )
print("LlamaForCausalLM.from_pretrained Success!")
model.resize_token_embeddings(len(tokenizer))
model = prepare_model_for_half_training(model=model,
...
model = model.cuda()
print("prepare_model_for_half_training Success!")
first_weight = model.model.layers[0].self_attn.q_proj.weight
first_weight_old = first_weight.clone()
model = PeftModel.from_pretrained(model=model,
                                  model_id=PATH_MODEL_LORA,
                                  # model_id=REPO_ID_LORA,
                                  # cache_dir=PATH_MODEL_LORA,
                                  # device_map={"": "cpu"},
...
try:
    for name, param in model.named_parameters():
        if name and ("lora_A" in name or "lora_B" in name):
            param.data = param.data.to(torch.half)
    print_named_parameters(model, use_print_data=True)
    ### lora merge-and-unload
    model = lora_merge_and_unload(model, first_weight, first_weight_old)
except Exception as e:
    print(traceback.print_exc())
    model = lora_merge_and_unload(model, first_weight, first_weight_old)
print("lora_merge_and_unload Success!")
print_named_parameters(model, use_print_data=True)
## train
model.gradient_checkpointing_enable()
model.enable_input_require_grads()
...
print("get_peft_model Success!")

# model = model.cuda()
print_named_parameters(model, use_print_data=True)
print("model.cuda() Success!")
...
# |myz|
"""
('base_model.model.model.layers.31.mlp.up_proj.lora_A.weight', torch.float32, True)
('base_model.model.model.layers.31.mlp.up_proj.lora_B.weight', torch.float32, True)
...
('base_model.model.lm_head.weight', torch.float32, True)
"""
