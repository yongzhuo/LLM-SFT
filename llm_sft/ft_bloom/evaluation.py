...
from llm_sft.ft_bloom.config import CUDA_VISIBLE_DEVICES, USE_TORCH, CPU_NUMS  # from config
...
from llm_sft.models.bloom.modeling_bloom import BloomForCausalLM, BloomPreTrainedModel
from llm_sft.models.bloom.tokenization_bloom_fast import BloomTokenizerFast
from llm_sft.ft_bloom.config import LEARNING_RATE, EPOCHS, SAVE_STEPS, VAL_SET_SIZE, TARGET_MODULES
from llm_sft.ft_bloom.config import MICRO_BATCH_SIZE, BATCH_SIZE, GRADIENT_ACCUMULATION_STEPS
from llm_sft.ft_bloom.config import PATH_MODEL_PRETRAIN, DATA_PATH, MODEL_SAVE_DIR, REPO_ID
from llm_sft.ft_bloom.config import MAX_LENGTH_Q, MAX_LENGTH_A, MAX_LENGTH_QA
from llm_sft.ft_bloom.config import LORA_DROPOUT, LORA_ALPHA, LORA_R
from llm_sft.ft_bloom.config import USE_CUDA
...
model = BloomForCausalLM.from_pretrained(PATH_MODEL_PRETRAIN)
print_named_parameters(model, True)
tokenizer = BloomTokenizerFast.from_pretrained(PATH_MODEL_PRETRAIN, add_eos_token=True)
...
def txt_read(path, encode_type="utf-8", errors=None):
    ...
    try:
        ...
        return lines
def predict(data_point, generation_dict):
    """  推理  """
...
if __name__ == '__main__':
    ...
    fw = open(DATA_PATH + ".bloomz_eval_rouge_blue.512.json", "a+", encoding="utf-8")
